...
void MyWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    if(i >= 4)
    {
        ...
        {
            x = pow(1-t,3)*tabx[0] + 3*pow(1-t,2)*t*tabx[1] + 3*(1-t)*pow(t,2)*tabx[2] + pow(t,3) * tabx[3];
            y = pow(1-t,3)*taby[0] + 3*pow(1-t,2)*t*taby[1] + 3*(1-t)*pow(t,2)*taby[2] + pow(t,3) * taby[3];
            pix(x,y);
        }
        update();
    }
